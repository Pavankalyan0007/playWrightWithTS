...
test("Tags2 in playwright", {tag:["@smokeTest","@RegressionTest"]}, async ({ page }) => {
                    test.slow();
                    await page.goto('https://www.youtube.com/');
                   const searchBox = await page.getByPlaceholder("Search");
                    await expect(searchBox).toBeEnabled();
                   ...
                   await searchBox.press('Enter');
                   await expect(page).toHaveURL('https://www.youtube.com/results?search_query=pavan+kalyan+soppa')
